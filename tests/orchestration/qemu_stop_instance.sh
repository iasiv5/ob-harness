#!/usr/bin/env bash
# tests/orchestration/qemu_stop_instance.sh — qemu_stop_instance orchestration。
...
# Case 2: 已退出进程(无 /proc)→ 不卡、清 PID 文件(qemu_stop_instance 仍 rm)
: &
pid2=$!; wait "$pid2" 2>/dev/null   # 让它退出
pf2="$(mktemp)"
qemu_stop_instance "$pid2" "$pf2"
