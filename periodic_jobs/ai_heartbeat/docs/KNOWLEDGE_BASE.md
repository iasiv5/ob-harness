...
- **操作文件**:
  1. **规则层 (L3)**: 直接根据最新观测到的有效规律、语言风格变化、以及长效约束，修改或更新 `rules/` 下的核心规则文件 (`01_SOUL.md`~`06_AXIOMS_INDEX.md`)，并在确有必要时更新或新建 `rules/knowhow/` 下的真实 know-how 文档。
  2. **Know-how 索引与新增规范**: 当晋升目标落在 `rules/knowhow/` 下的真实 know-how 文档时，必须遵循以下规则：
