#!/usr/bin/env bash
# lib/verify_assertions.sh — ob verify 的断言判定函数族(leaf-pure)。术语见 CONTEXT.md function semantic layer / ob verify.
#   每个函数消费一个接口的原始信号(HTTP code+body / ipmitool rc / tcp rc),return 0=pass / 1=fail,
#   并向 stdout 打印一行 ✓/✗ + 断言名(+ 失败时附简短诊断)。
#   调用方(cmd_verify, lib/qemu_commands.sh)在 fail 时额外打印该接口的完整 RAW response 做定位。
#   原始信号的采集(probe: curl/ipmitool/tcp)归 exit-seam(cmd_verify 私有 _verify_probe_*);本 module 只判。
...
#   中 verify_assertions.sh 例外集空——判函数必须不 exit, 把 exit 收口留 exit-seam cmd_verify）。


# verify_judge_redfish_root <http_code> <response_body>
...
# verify_judge_ipmi_lan <ipmitool_rc> [<output_excerpt>]
...
#   正确性由 tests/protocol/verify_assertions_judgment.sh stub-喂 rc=0 验证。lib/qemu.sh:29 已正确
#   发 hostfwd=udp::ipmi_port-:623, 故非转发问题, 是 image 侧缺 netbridged。
verify_judge_ipmi_lan() {
    local rc="$1"
    local excerpt="${2:-}"
    if [[ "$rc" == "0" ]]; then
