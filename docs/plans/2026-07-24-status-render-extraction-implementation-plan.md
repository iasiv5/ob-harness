...
- 迁移用全限定名 + 调用点双向核对（F7 防 grep 漏，正则用 `[a-z0-9_]+`）：`status_section_*` → `status_render_*` 重命名后，`grep -rn 'status_section_' lib/ tests/ tools/ --exclude-dir=archive`（排除 `workspace/` 与归档死代码 `tools/archive/`）须清零。`tools/archive/reorder.py` 含历史 `status_section_*` 名是已归档死代码（00_reorder 由文件边界接管），非残留，故排 archive。
