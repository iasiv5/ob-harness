...
- **测试（实施计划落地）**：新 unit 矩阵（cli_first：CLI set/unset × old set/unset × HTTP none 变体）；protocol surface gate 锁 production 不再内联注入 ritual（interface-shrink，bestpractice_10 形态 A）+ 锁两调用点都经 module（防对称化/内联回潮）；既有 `tests/protocol/qemu_restart_port_reuse.sh`（改写为值断言）+ `tests/orchestration/start_qemu_force_restart.sh`（F1 顺序不变量）保持绿。实现评审加固：unit 补四端口 cli_first 对称锁、surface forbidden 扩四端口族、restart gate 补 stale_seg non-empty 守卫、新增 `tests/orchestration/deploy_to_qemu.sh` 场景⑧（deploy honor CLI 行为级锁，对 unit×surface 组合的运行期 composition 加固；run_deploy 跳过 parser，故不覆盖 parser→`QEMU_SSH_PORT` 接线）。
