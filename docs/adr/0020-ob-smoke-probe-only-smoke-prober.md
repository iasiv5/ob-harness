...
- **future-candidate（不违本 ADR）**：(1) smoke 无 machine arg 时交互选**在跑**的 instance（qemu_instance 列表）；(2) 真机 BMC target（非 QEMU）；(3) 断言扩深（Redfish Managers / sensor）；(4) baseline-diff 伴生（CI 闸门闭环）。
