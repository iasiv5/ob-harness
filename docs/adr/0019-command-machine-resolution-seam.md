...
- **`exit_on_user_cancel` 退役**：4 命令（含 start_qemu）的 rc 映射全迁字面 case + cancel warn（warn 不丢、搬进 seam/caller，对齐 intake 派；护 `"Build cancelled by user."` 断言（已随 cancel-warn 迁至 `tests/unit/machine_resolve.sh` + `tests/orchestration/build_confirm_cancel.sh`；原 `tests/unit/interact.sh:21` 已不是该断言））。函数从 `lib/commands.sh` 删除。
