...
🔴 High: [ob 优先约定确立] 确立"ob 优先"作为 OpenBMC 环境动作的统一前门：agent 做 OpenBMC 生命周期动作（init/build/status/start-qemu/stop-qemu）前，先查 `ob --help`（唯一权威能力清单，由 `tests/protocol/usage_dispatch_sync.sh` 断言与 dispatch 一致防漂移），提供就走 `ob <cmd>`，不手动兜底。退出码回退语义固化：exit 1=真失败才考虑手动、exit 2=用户取消、exit 3=前置缺失用 ob 补前置（禁止据此转手动）。两层落地：常驻守卫在 `AGENTS.md` Working Mode，完整协议在 `rules/knowhow/bestpractice_06-ob_first.md`。配套 ADR-0003 与 `#remedy line`（exit 3 恒带一行含字面 ob 命令的补救提示，术语登记在 `CONTEXT.md`）。
...
🔴 High: [重构/优化的回归锁方法论落地] 两处同族实践。(a) profile 重构配结构回归锁 `tests/protocol/qemu_launch_profile_structure.sh`：锁旧 public 调用清零、下游不再读旧决策变量、`build_qemu_cmd` 不再查 bootloader、`cmd_start_qemu` 必调新 interface，防收敛后旧路径悄悄回潮。(b) 新 know-how 条目 `rules/knowhow/bestpractice_09-nonfunctional_regression_locks.md`（commit 3878f7e）把“不改输出但声称优化”的改动（perf/去重/缓存）钉成可回归硬约束：模式1 调用次数断言（恰 N 次）、模式2 零调用断言（被消除函数计数文件不存在）。bash 手法：monkey-patch `eval "$(declare -f orig | sed '1s/^orig/_shadow/')"` 重定义 wrapper 计数（关键是 `1` 只改首行防误伤 body，`^` 是保险）；计数写文件不写变量（子 shell `$()`/`< <()` 内变量累加会丢，与 bestpractice_07 同源）；期望值钉死 `==N` 非 `>=N`。适用边界：输出断言已能区分对/错时不再加调用次数断言（避免耦合实现细节）。上游公理 V2 可验证性 / V6 概率乘。
...
🟡 Medium: [bestpractice_10 抽取族形态 A-E] rules/knowhow/bestpractice_10-deep_module_extraction.md 把 5 次同构抽取提炼为 5 收敛形态:A lifecycle state(machine_state,谓词式接口调用者只问不查)/B profile(qemu_launch_profile,统一决策变量命名空间 + 入口 reset 防跨用例泄漏)/C picker(machine_picker,退役多处重复选择逻辑)/D instance(qemu_instance,best-effort 恒 rc 0)/E god-function 拆(薄 L1 wrapper + runtime 深 prepare/execute),配 leaf-pure 静态门禁(exit_contract.py Y 规则按 basename 配例外集)、extract→pin→deepen 顺序、退役 grep 全限定名自验证、已知陷阱表(F1 跨 seam 副作用次序/F7 函数名前缀让 grep 漏/工具依赖单文件假设漂移如 radar/跨用例状态泄漏/把单点当模式硬抄需 ≥3 同构才固化)。与 codebase-design DEEPENING(通用 module/seam 词汇)互补不重复。bare_mirror 是 know-how 形态化后首个应用 + 阈值第 6 例。
...
🔴 High: [command machine resolution seam 收口——machine 解析 ritual 收为单一 leaf-pure 深模块] 08-01 PR#35（ADR-0019 路A）新增 lib/machine_resolve.sh（leaf-pure，exit_contract.py Y 白名单 `'machine_resolve.sh': set()` 已登记）的 `resolve_command_machine`，把 cmd_build/cmd_dev/cmd_deploy_to_qemu（以及 07-30 收口的 cmd_start_qemu/cmd_stop_qemu）各自重复的 machine 解析 ritual（given 快路径 verify / empty 路径 guard+pick / exit-3 remedy / rc 映射）收成一个入口，return 0/1/2/3 契约 + set $MACHINE。源码复核要点：(1) given 路径（$MACHINE 非空）own verify——通过 return 0/未通过 verify remedy+return 3；empty 路径经 machine_selection_guard outvar 回传 empty/nontty/ok，pick_machine 自 initialized_machines 源可信不重复 verify；(2) cancel 不再靠 exit_on_user_cancel helper（grep 证 ob/lib 零残留，commit 04fc83f 退役），改 caller 字面 case+warn 收口 exit（续 07-23 exit_contract X 禁 dynamic exit 纪律）；(3) 后置条件 return 0 ⟹ $MACHINE initialized。是 06-24→07-12 深模块抽取族（bestpractice_10）在 cmd_* 编排层 machine-selection 维度的集中收口；07-29 init_intake + 07-30 qemu guard 是本 seam 的前置局部收口。
🔴 High: [长文件 TL;DR 强制门禁 + 漂移 advisory（ADR-0015）] 07-29 PR#32 给 know-how 体系加结构门禁：长文件（>100 行）顶部必配 TL;DR（30 秒核心判定），由 tools/ob_check.sh 强制（ob_check 每跑必检长文件 TL;DR 存在）；配套 tools/knowhow_tldr_drift_check.py 做 git diff 结构漂移 advisory——长文件正文改但 TL;DR 没改→flag 疑似漂移（不阻断，advisory 形态）。消费者义务（05_KNOWHOW_INDEX 消费速则）与生产者义务（ADR-0015）对偶锁死。这是 bestpractice_08 eval 门禁模式库的延续——把"消费者只读前半截就动手是高发陷阱"从 convention 升为可机器验证门禁。同步 bestpractice_01 写作指南 + 多篇长 know-how 补 TL;DR + 尾段（已知陷阱）归位。
🔴 High: [编号稳定性——永久序号删后留缺口不重排（ADR-0018）] ADR-0018（07-31）确立 know-how 文件 `<NN>` 是永久创建序号：删除条目后编号不回收、不重排填补，缺口永久保留（故 workflow 分类是 01/02/04——03 已删属正常）。可复用方法论：稳定标识符 > 整洁序列——外部引用（Agent 回指、INDEX 入口、ADR 指针）在重排时连锁断裂，留缺口成本低于重排的批量更新。与 CONTEXT.md 领域术语硬切迁移（06-24）同属"标识符稳定性"决策族。
🟡 Medium: [know-how 沉淀机制优化——product/user 分发边界 + 手动沉淀编排层（ADR-0017）] 07-30 commit b1b4eb2 明确双层分发边界：product 层（rules/knowhow，随上游分发）vs user 层（contexts/knowhow，.gitignore 不回上游，索引 07_USER_KNOWHOW_INDEX.md）。reflector GC/晋升天花板显式限 rules/+OBSERVATIONS，禁止碰 contexts/knowhow（会把 user 内容推上游击穿 git 边界）；observer 可扫描 contexts/knowhow 提示用户手动沉淀（workflow_04 + /sediment），但不作 reflector 输入。新增 bestpractice_12 知识分层判定（三问路由：源于 ob-harness 自身且别人会复发→product；纯环境/个人健忘→会话记忆）+ workflow_04 手动沉淀编排层。05_KNOWHOW_INDEX 加每会话 user 层注入硬要求。
🟡 Medium: [init_intake + QEMU commands guard 前置收口] (1) 07-29 PR#33 抽 lib/init_intake.sh leaf-pure（dev_intake_argv 纯 argv 解析 + dev_intake_tty TTY 菜单引导，与 devtool_intake 同构，ADR-0010/0012）；guard 第 3 消费暂缓记 ADR-0016（避免在未稳定的 guard 接口上过早耦合）。(2) 07-30 PR#34 cmd_start_qemu/cmd_deploy_to_qemu 的 machine-selection 序言走 machine_selection_guard 收口，配 qemu_commands_guard_surface protocol 锁。两者是 08-01 machine_resolve seam 的前置——先局部收口再统一进 resolve_command_machine。
🟡 Medium: [/goal-driven slash command + devtool_porcelain 失败路径测试] (1) 08-02 PR#37 新增 /goal-driven 模板组装器 slash command（Claude Code .claude/commands + Copilot .github/prompts 双入口）。(2) 08-02 e628b94 补 devtool_emit_json 非法 JSON 失败路径单测（锁 porcelain emit 原语对坏输入的防御，coverage 硬化）。
🟢 Low: [housekeeping] (1) 08-02 f0dc377 订正 4 个 ADR 滞后代码指针 + bestpractice_10 加 ADR 指针同步验收项（文档/代码漂移修复，与 ADR-0015 TL;DR drift 同族）；(2) 08-01 PR#36 GitHub code scanning alert——workflow 加 permissions 字段（CI 安全硬化）；(3) .claude/skills/ 下大量 SKILL.md/results.tsv/test-prompts.json 系 darwin/grilling skill 运行副产物，git log 无相关提交，过滤不记（同 07-23 判定）。
