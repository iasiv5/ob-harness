...
🟡 Medium: [smoke 套件并入 test-qemu 落地 main（PR #46, 8ae0d50, 79 files +3907/-2422）] smoke 可达性门体系收编进 test-qemu baseline 框架：SMOKE-08 Web 登录 + 登录后会话资源可达门收尾（Web 登录返 201 的 bytedance 定制语义）；probe 类型体系扩展 + romulus baseline 重构；smoke 专属 verdict/exit_contract/coverage 登记项随之合并裁撤（smoke_verdict.sh 单测删除、coverage_matrix/exit_contract 同步收缩）。同批 1d16899 补齐 test-qemu help Prerequisites/Examples + ob --help 命令文案 agent 化（lib/qemu_commands.sh、ob 各有改动，tests/protocol/test_qemu_surface.sh 锁 help 语义）。
🔴 High: [CodeQL 数据流告警修复方法论已固化为 bestpractice_15] PR #46 三轮 CI 返工（probe_console.py clear-text logging #4/#5）教训已入 rules/knowhow/bestpractice_15-codeql-alert-taint-path-first.md：先拿告警页 Show paths 污点路径再定点切断（源值不进拼接/日志参数），四大陷阱——按名字定源不看值（env 名含 PASSWORD 即敏感源）、行内豁免注解不可靠、regex/replace 净化是污点保持、盲改净化反而新增污点路径。终态修复 = 诊断消息不再拼接 password_prompt 值（51c327e）。索引已更新（05_KNOWHOW_INDEX.md）。
🟢 Low: [周边同步] rules/03_WORKSPACE、workflow_01-obmc_env_init、bestpractice_06-ob_first 有小幅随动更新；本轮扫描 contexts/knowhow/ 无新变更，无手动沉淀提示。工作区 clean。

Date: 2026-08-28

🟡 Medium: [ob init romulus 6/103 mirror 失败清零——数据中心 IP 被 FOSS 站点封禁的完整处置] sourceware 403×3(binutils-gdb/glibc/bzip2-tests)/savannah 连接超时(config)/codeberg 显式 IP-block(xmlto)/salsa 429 无 Retry-After(debianutils,≥3h 不解除),重试全部无效。修复=(1) lib/bare_mirror.sh rewrite 表 +5 条:RTEMS/sourceware-mirror-binutils-gdb、kraj/glibc、CodeLinaro-mirror/qrdk_bzip2-tests、arthenica/gnu-config、pagure.io/xmlto(codeberg 迁移前上游仍在同步),每条入表前先 GitHub API /commits/<SHA> 或 /tmp 克隆 cat-file 验证含 pinned SRCREV;(2) debianutils 无任何可达 mirror 含 debian/5.23.2→Yocto 官方 git2 tarball 手动种子(downloads.yoctoproject.org/mirror/sources/git2_<gitsrcname>.tar.gz,master tip 恰=pinned SHA)。终态 init 103/103、report 无 Failed 段,六仓 pinned SHA 逐一 cat-file 硬验证。方法论已沉淀 workflow_01 故障排除 #2b/#2c + 已知限制表。ob 待补项:rewrite 表只支持 URL→URL,缺 tarball seed 能力(手动兜底已按 bp_06 记录)。
🔴 High: [ob_check shellcheck fail-open 实测发生——门禁把错误文本写进权威 baseline] 本环境缺 shellcheck 时跑 ob_check:line164 `|| true` 吞 "command not found",parse 出空 Counter 与非空 baseline 差异被误判 REGEN,直接 cp 错误文本进 tests/.shellcheck-baseline(靠 git diff 抓到,已还原)。修复=fail-closed 卫语句(command -v shellcheck 失败→bad+跳过比对与重生成,提示 apt 安装);另 sudo apt install shellcheck 0.9.0 修掉 test_runner_contract 的环境性失败。bp_07/bp_08 的活实例:缺工具时门禁必须显式失败,静默降级=把污染写进基线,此后每次比对都被污染基线架空。
🟢 Low: [rewrite 表 scheme 全等陷阱] bare_mirror URL rewrite 表与 deps.json clone_url 逐字匹配:recipe 带 protocol=https 的 clone_url 已归一为 https://(sourceware/savannah/codeberg 系全如此),不带 protocol 的保留 git://(infradead)——照抄既有条目的 scheme 形式会静默不命中、走原 URL(仅 -v 日志可见)。首轮部署即踩,靠 "Creating bare mirror: https://sourceware..." 日志抓到后改 https 形式。已记 workflow_01 #2b。
