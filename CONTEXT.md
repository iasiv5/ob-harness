...
**baseline (开发基线 / 功能基线)**:
一份 BMC 固件开发基线文档（原始形态常为团队维护的 Excel），由若干带编号的需求条目（AR，如 `BMC-3-1-2` 密码规则）组成，是 `ob test-qemu` 在 QEMU 上做一致性验证的需求来源。每个 machine 的 baseline 自成体系、按项目独立迭代——不共享 AR 定义、不共享 probe 引擎（全栈 per-machine，见 [ADR-0025](docs/adr/0025-test-qemu-baseline-fullstack-per-machine.md)）；落点按分发边界二分：社区基线 `tests/baseline/<machine>/`（随 ob-harness 上游），custom 基线 `contexts/baseline/<machine>/`（不随上游，接 [ADR-0017](docs/adr/0017-knowhow-distribution-boundary.md) 的 `contexts/` 边界）；`ob test-qemu` 按被测对象谱系硬路由二选一，不跨谱系回退（[ADR-0026](docs/adr/0026-test-qemu-baseline-lineage-routing.md)）。QEMU 只覆盖其可仿真子集（纯软件 / 协议 / 网络档），硬件 / host 依赖档留给未实现的 `ob test-sut`。与 `ob smoke`（浅冒烟 5 条哨兵）正交：smoke 守 per-push 绿灯、零 per-machine 知识（ADR-0020），test-qemu 是逐条 pass/fail/skip/xfail/xpass 的深测（xfail 预期失败、xpass 意外通过，均不影响 exit）、全栈 per-machine。
