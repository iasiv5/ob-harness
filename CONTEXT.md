...
**ob smoke**:
ob 对一个**已在跑**的 `QEMU instance` 做带外（OOB）接口可达性 smoke 探测的独立顶层命令（与 `ob start-qemu` / `ob stop-qemu` / `ob deploy-to-qemu` 同族，**非 `ob dev`**）。**probe-only**：不拥有 QEMU bring-up / teardown（那是 start-qemu / stop-qemu 的职责），前置是目标 machine 的 `QEMU instance` 正在跑——从 `QEMU PID file` 读真实转发端口，跑确定性断言（Redfish 根可达 / Redfish Managers 资源 / Redfish SoftwareVersion 固件版本上报 / IPMI over LAN / system-ready 共五条，其中 Redfish 三条由两个 probe 喂三个 leaf-pure judge），每条 ✓/✗ + 失败原始返回，全过 exit 0、任一失败 exit 1（`exit-code 契约` 0/1/2/3）。**peer 命令不是 sub-step**：任何 ob 命令不得调用它、不得在 `remedy line` 里 nudge 向它（解耦原则）；smoke→start-qemu 的前置 remedy（`Run './ob start-qemu <machine>' first`）合法，反向 nudge 不合法。verdict 是**纯报真相**（strict 全过），**不拥有**任何 per-machine「预期 OOB profile」知识——回归判定（baseline diff）归 caller：伴生工具 `tools/smoke_diff.py` 对两次 smoke 输出按断言名配对，把 `✓→✗`（同名退化）与 baseline 无此名 + current ✗（新出现的失败断言）两类都判回归（exit 0 放行 / exit 1 拦截）；`✗→✓` 改善仍作 info。temporal CI 闸门 `tools/smoke_regression.sh <machine> -- <change-cmd...>` 一键跑改前/改后——校验在跑实例 → 捕获 baseline smoke 快照（**运行时临时产物 `mktemp`，非版本管理**）→ 执行 `<change-cmd>` → 重采 current → 调 smoke_diff.py → 按 exit 0/1 透传为 gate 的 exit 0/1（exit 3 = 无在跑实例，透传不当中 gate 失败）。gate 不拥有 QEMU 生命周期（同 smoke probe-only）；baseline/current 均为运行时临时产物，零 per-machine 知识（ADR-0020 option-1 temporal diff 范畴，非 option-3 spatial 期望）。服务 CI 闸门与 agent 改前/改后比对。死/stale `QEMU instance`（存活状态非 `running`）→ `qemu_instance_clean_stale` + exit 3（绝不探死端口报假「BMC 坏」）。自带 sshpass-independent TCP 就绪门（轮询 SSH 转发端口，带上界 `OB_SMOKE_READY_ATTEMPTS`）——`ob start-qemu` 默认 BMC-ready 等待 sshpass-dependent 且超时只 warn，不足以做 smoke 的探测定时门。与 `ob deploy-to-qemu`（image 级重建+重启）正交组合成 E2E：`ob deploy-to-qemu <m>` → `ob smoke <m>`（smoke 验的是**在跑的实例**，不是刚 build 的 image——后者须先 deploy）。已知环境约束：gb200nvl-obmc image 不含 RMCP+ LAN responder，smoke 上其 IPMI 断言合法 ✗ + exit 1（真相非失败，非测试缺陷）。决策见 [ADR-0020](docs/adr/0020-ob-smoke-probe-only-smoke-prober.md)。runner/spec 抽取暂缓（非同构 5 段，等第二个 adapter），见 [ADR-0023](docs/adr/0023-defer-smoke-assertion-runner.md)。
_Avoid_: ob verify（已改名；"verify" 过载——旧 `ob verify` 指 bring-up+teardown 的初版，已退役为 probe-only smoke）

**ob test-qemu**:
ob 对一个**已在跑**的 `QEMU instance` 逐条跑该 machine 的 `baseline`（开发基线）里 QEMU 可仿真子集 AR 的独立顶层命令，与 `ob smoke` 同族同 probe-only 形态、**正交姊妹**。**probe-only**：不 boot / 不 teardown（无 EXIT trap）。前置集按模式分叉：probe 模式前置是目标 machine 的 `QEMU instance` 正在跑——从 `QEMU PID file` 读真实 Redfish 转发端口（不接 port override，反映实际跑的实例）；`--dry-run` 是 baseline 资产检查（列 AR + applicability，不 probe），前置集仅 {machine、PyYAML、baseline 谱系+目录}，无需在跑实例、无需凭据（runner `DRY_RUN` 分支同构豁免）；probe 模式下无在跑实例（存活状态非 `running`）→ `qemu_instance_clean_stale` + exit 3 + `remedy line`（`Run './ob start-qemu <machine>' first`，绝不探死端口报假「BMC 坏」）。**baseline 目录谱系路由**（per-machine 全栈独立，[ADR-0025](docs/adr/0025-test-qemu-baseline-fullstack-per-machine.md) + [ADR-0026](docs/adr/0026-test-qemu-baseline-lineage-routing.md)）：谱系 = source label 单维度（唯一权威事实源——binary 目录/provisioning 均由 label 派生共线，binary 不构成独立信号）；label 缺失（`source manifest` 缺失/source_label 字段空）**不 fallback community**，判 unknown → exit 3 + 按成因 remedy（与写坏同档 fail-closed 强防御；binary provisioning 等其他 label 消费方的 community 缺失默认不受影响）；community 谱系 → 只查 `tests/baseline/<machine>/`（社区基线，随上游），custom 谱系 → 只查 `contexts/baseline/<machine>/`（custom 基线，不随上游），**各找各的、不跨谱系回退**，本谱系目录缺失 exit 3 + 按谱系指名 remedy；谱系判定与路由由 leaf-pure helper `test_qemu_lineage` / `test_qemu_resolve_baseline_dir` 实现（恒返回码 + outvar，对齐 `machine selection guard` / [ADR-0024](docs/adr/0024-qemu-instance-liveness-outvar.md)，可被 protocol 层直测、不必构造 fake alive QEMU）。路由命中**纯数据目录**后调主仓单副本 `tests/baseline/runner/run.sh`（薄 shim，编码钉死后 `exec python3 runner.py`；runner 引擎共享单副本，machine 差异只在数据 YAML，见 [ADR-0027](docs/adr/0027-baseline-runner-single-copy-in-main-repo.md)；**不复用** `ob smoke` 的 probe 原语），数据路径由 cmd 以 `OB_TQ_AR_PROBES`/`OB_TQ_APPL` env 注入，auth 解析 env 优先（`OB_TQ_USER`/`OB_TQ_PASSWORD`，凭据全程不落 argv/`ps`，environ owner-only）、`ar_probes.yaml` 顶层 `auth:` 补缺（不硬编码凭据），host/port 由 cmd 注入；两份 YAML 顶层须声明 `schema_version`（plan.py 校验，不匹配 exit 3——runner 与数据分属两仓的版本门禁）；custom 谱系永不消费社区基线——custom build 的 fail 无法归因到上游（是上游也这样，还是 fork/build 特有？），路由层直接杜绝该错配，归因恒闭合。逐条 AR 经 probe + assert 原语判定（**runtime 确定性**，LLM 不参与判定），产 `pass` / `fail` / `skip` / `xfail` / `xpass` 五态 verdict（另：probe 取不到干净 BMC 应答的传输/超时/schema 错误记 `error`，属 infra、**不进 `fail` 的 α 统计**，见 exit 3）：`skip`（不适用 / 硬件依赖，**不调 probe**）、`xfail`（预期失败，**调 probe**：预期 fail→xfail、意外 pass→xpass，xpass 是改善信号）、`skip`/`xfail`/`xpass` **均不影响 exit**——只有 applicable 且实测不符的 `fail` 才 exit 1。`exit-code 契约` 0/1/2/3：0 全 applicable AR pass；1 ≥1 applicable fail = **α truth**（BMC 不满足 baseline 的真实一致性缺陷，**非 test-qemu broken**）；2 cancel；3 前置缺失（无在跑实例 / machine 未解析 / baseline 目录缺失）+ 一行 remedy，**或运行中 probe 命中瞬时 infra 错误（传输/超时/schema，VERDICT: ERROR）——此时实例未必坏，重跑 / 查连通性而非当 baseline 缺陷**。与 `ob smoke`（浅冒烟 5 条哨兵、零 per-machine、守 per-push 绿灯）正交：test-qemu 是逐条深测、全栈 per-machine、nightly / PR-to-main 频率。LLM 角色限离线 authoring（读 Excel baseline → 翻译 AR 数据）与失败解说（fail 行自然语言解释），**均超出 spike 范围**（runtime 不参与判定）。
_Avoid_: conformance（弃用，见 `baseline` 词条 _Avoid_）, ob test-qemu verify / ob verify baseline（"verify" 过载，见 `ob smoke` _Avoid_）, 把 exit 1（α truth）当 test-qemu broken（fail 行报的是 BMC 实际行为，debug BMC 接口而非测试）

**baseline (开发基线 / 功能基线)**:
一份 BMC 固件开发基线文档（原始形态常为团队维护的 Excel），由若干带编号的需求条目（AR，如 `BMC-3-1-2` 密码规则）组成，是 `ob test-qemu` 在 QEMU 上做一致性验证的需求来源。每个 machine 的 baseline 数据自成体系、按项目独立迭代（不共享 AR 定义，见 [ADR-0025](docs/adr/0025-test-qemu-baseline-fullstack-per-machine.md)）；runner 引擎则为主仓单副本 `tests/baseline/runner/`（`tests/baseline/runner`，machine 差异只在数据 YAML，[ADR-0027](docs/adr/0027-baseline-runner-single-copy-in-main-repo.md) 修订 0025 的引擎不共享结论）。数据落点按分发边界二分：社区基线 `tests/baseline/<machine>/`（随 ob-harness 上游，**子仓纯数据**），custom 基线 `contexts/baseline/<machine>/`（不随上游，接 [ADR-0017](docs/adr/0017-knowhow-distribution-boundary.md) 的 `contexts/` 边界；只承载数据 + 数据生产线，零运行时代码）；`ob test-qemu` 按被测对象谱系硬路由二选一，不跨谱系回退（[ADR-0026](docs/adr/0026-test-qemu-baseline-lineage-routing.md)）。两仓（runner 主仓 / custom 数据子仓）由 YAML 顶层 `schema_version` 门禁耦合。QEMU 只覆盖其可仿真子集（纯软件 / 协议 / 网络档），硬件 / host 依赖档留给未实现的 `ob test-sut`。与 `ob smoke`（浅冒烟 5 条哨兵）正交：smoke 守 per-push 绿灯、零 per-machine 知识（ADR-0020），test-qemu 是逐条 pass/fail/skip/xfail/xpass 的深测（xfail 预期失败、xpass 意外通过，均不影响 exit）、全栈 per-machine。
