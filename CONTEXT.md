...
cmd_*（L1 exit seam）在交互选 machine 前对 `machine selection`（`pick_machine`）两条前提——machine 集合非空、当前为交互终端——的检测 module（`lib/machine_selection_guard.sh`，函数 `machine_selection_guard`）。它把这两条前提从 cmd_build / cmd_dev / cmd_start_qemu / cmd_deploy_to_qemu 各自内联的「枚举集合 + 空 guard + TTY guard」编排收口到单一 module，以 outvar status 回传（empty=集合空 / nontty=集合非空但非交互终端 / ok=非空且交互终端），恒返回 0；绝不 exit、不打印 `remedy line`、不做展示、不选号——exit/remedy/展示归调用方（cmd_* 据 status 做 display + exit 映射），选号归调用方在 ok 后调 `pick_machine`。leaf-pure L3。它对应 pick_machine 文档前提（"调用者保证集合非空 + 交互终端"）的检测部分。与 `machine selection`（pick_machine 纯选号原语）正交：guard 检测前提，selection 在前提满足后选号；二者不合并进单一 module（guard 是检测原语，selection 是选号原语，cmd_* 编排两者）。勿与 `resolve_machine`（init 已退役旧函数，含 arg 快路径与 confirm）混淆。消费方：cmd_build / cmd_dev / cmd_deploy_to_qemu 经 `resolve_command_machine`（见 `command machine resolution` / [ADR-0019](docs/adr/0019-command-machine-resolution-seam.md)）间接消费 guard（seam 内部调 guard 做 empty/nontty/ok 检测），cmd_start_qemu 仍直接消费 guard；cmd_stop_qemu 选 running instance、不经 guard。cmd_start_qemu 的 empty 分支带 any_initdone 子分类（image-ready 空时再查 initialized 区分 remedy「先 build」vs「先 init」），留 cmd 层（guard 是横切检测原语，不承载 image-ready 领域逻辑，D2）。
_Avoid_: machine guard（口语化，语义太泛）, 把 guard 当 selection（前者检测前提，后者选号）, 把 guard 当完整 machine 解析（module 只检测 empty/nontty，不含选号；选号留调用方调 pick_machine）, machine resolution（曾误指本 guard，已改：guard 只检测前提不做完整解析；`command machine resolution` 另指 L1 machine 解析编排 seam，见该术语 / [ADR-0019](docs/adr/0019-command-machine-resolution-seam.md)，二者勿混）

**command machine resolution**:
`cmd_*`（L1 exit seam）在动作前把「本命令对哪个 machine 操作」解析为「一个已 initialized 的 machine」的 leaf-pure 编排 module（`lib/machine_resolve.sh`，入口 `resolve_command_machine`）。它消费 `machine selection guard`（empty/nontty/ok 检测）与 `machine selection`（pick_machine 选号），把「given 快路径 verify / empty 路径 guard+pick+verify」两路编排 + verify-init 前置 + cancel warn + rc 组合收成单一 module；return `exit-code 契约`值 0/1/2/3（0=解析成功，`$MACHINE` 已 initialized / 2=用户取消，已 warn / 1=读失败 / 3=前置缺失，已打 remedy），exit 收口独占留调用方 `cmd_*`（字面 case 映射，对齐 `ob dev command intake` / `ob init command intake` 的 intake 派）。沿用 `$MACHINE` 全局（pick_machine 本就 set 它，不造 outvar 假对称，同 init intake 决策）；后置条件：return 0 ⟹ `$MACHINE` 已 initialized。它把 `machine selection guard` 术语曾明写的「exit/remedy/展示归调用方」迁移为「resolution module own remedy + return 契约，cmd_* case 收口」（intake 派），见 [ADR-0019](docs/adr/0019-command-machine-resolution-seam.md)。与 `machine selection guard`（检测前提原语）/ `machine selection`（选号原语）正交：guard 检测、selection 选号、resolution 编排三者不合并。confirm / repo 显示 / DRY_RUN / 展示块**留调用方**（关于动作风险与展示，非 machine 解析）；pick 输出流（stdout/stderr——dev 须 stderr 护 `ob dev porcelain stdout` 契约）与 nontty remedy 文案（CLI 形态各异）作参数传入。v1 消费方为 cmd_build / cmd_dev / cmd_deploy_to_qemu 共享（initialized 命令族）；cmd_start_qemu 的 image-ready 协议（empty 子分类 + image-file 前置）暂不接本 seam，留 future-candidate（见 ADR-0019）；cmd_init 永久 out（[ADR-0016](docs/adr/0016-defer-init-intake-guard-reuse.md)）。
_Avoid_: 只写「machine resolution」不写全称（与 guard 曾误用的 resolution 撞，须写 `command machine resolution` 消歧）, 把 resolution 当 selection 或 guard（三者正交：选号 / 检测 / 编排）, 让 resolution own confirm 或 repo 显示（动作风险与展示留 caller）, 给 resolution 加 nameref outvar 回填 machine（沿用 `$MACHINE` 全局）
...
**QEMU lifecycle lock**:
`workspace/qemu-bin/.locks/<machine>.lock` 上的 per-machine flock，串行化同一 machine 的 QEMU instance writer（`ob start-qemu` / `ob stop-qemu` / `ob deploy-to-qemu` 与拥有测试生命周期的 integration），并在 `ob test-qemu` 探测期间 pin 住目标实例。它覆盖 instance 检查、启动/停止和 PID manifest 发布，防止多用户同时看到 `nopid` 后双启动、覆盖 PID manifest、check→stop 间误杀替换实例，或 probe 报告混入重启前后两个实例的响应。锁文件本身不承载状态；状态事实仍只来自 `QEMU PID file` 与进程。父编排可持锁并把 FD 继承给 child ob，child 验证 lock path 后在命令期间继续持有；仅 QEMU daemon-side copy 在 setsid 前关闭，避免 daemon 延长锁生命周期。
...
**重启 (restart)**:
`ob start-qemu` 的一种 QEMU lifecycle declaration：同 machine 存在 running `QEMU instance`（存活状态为 `running`）且本次会 kill 它后重新拉起。它是"原位替换"语义——默认沿用即将被 kill 的旧实例的端口（见 `端口解析链`），区别于"首启"（无旧实例）与"stale 清理"（存活状态为 `exited`/`recycled`，端口可用前提已崩，不沿用）。`restart` 仅发生在 `cmd_start_qemu` 的**交互确认分支**（running + 用户敲 y）；`--force` **不是** restart（它是 confirmation-gate modifier，修饰"是否问 y/N"，非 lifecycle declaration，见 `--force`）。决策见 [ADR-0021](docs/adr/0021-qemu-restart-port-reuse.md)。
_Avoid_: kill-and-restart（UI 文案，术语用 restart）, reboot（与 BMC reboot 混）, 把 `--force` 当 restart

**端口解析链 (port resolution chain)**:
`ob start-qemu` / `ob deploy-to-qemu` 决定 QEMU 生效转发端口（`QEMU_LAUNCH_SSH_PORT` / `QEMU_LAUNCH_REDFISH_PORT` / `QEMU_LAUNCH_IPMI_PORT` / `QEMU_LAUNCH_HTTP_PORT`）的有序来源。实际生效序（注入到 CLI flag 层使旧实例端口高于 env）：`本次 CLI flag（--ssh-port / QEMU_SSH_PORT）> [restart-only] 旧实例端口（PIDFILE_SSH_PORT）> 本次 env var（OB_QEMU_SSH_PORT）> 默认值（SSH 2222 / Redfish 2443 / IPMI 2623；HTTP 无默认、opt-in）`。restart-only 的旧实例层**仅**在 `重启 (restart)` 交互确认路径注入（`-z` guard 到 `QEMU_*_PORT`），首启 / stale 清理 / `--force` 路径该层缺失、直接 fallback 到 env/默认。注入机制的演进（start/deploy guard 的历史不对称及其由 leaf-pure module 统一）属实现细节，见 [ADR-0021](docs/adr/0021-qemu-restart-port-reuse.md) / [ADR-0022](docs/adr/0022-port-reuse-resolver-module.md)。`端口解析链` 把"端口来源"显式化为独立领域概念，防止把"默认值漂移"误读为"端口冲突"。
_Avoid_: 端口选择（口语化）, port fallback（用链强调有序性）, 把链序写成 env > 旧实例（注入到 CLI flag 层使旧实例物理上高于 env——旧实例 > env 是 path-X 注入的机制结果、非设计目标，见 ADR-0022 future-candidate）, 把 PIDFILE_* 当解析链一层（它是旧实例端口的物理载体，不是链上独立来源——经 `-z` guard 注入到 CLI flag 层才进链）
...
**QEMU launch profile**:
`ob start-qemu` 启动某个 machine 前解析出的启动画像，汇总 SoC 类型、证据来源/置信度、QEMU system binary name（`QEMU_LAUNCH_SYSTEM_NAME`，来自 `QB_SYSTEM_NAME` 或 profile 推导）、QEMU machine 名、内存参数以及 AST2700 额外启动文件需求。QEMU machine 名解析链：优先 `QB_MACHINE`（qemuboot.conf deploy 产物 > `bitbake -e`），两者皆缺时走 legacy machine-name fallback 派生 `<prefix>-bmc`；ob **不**依据已安装 binary 支持的机型改写该值——机型事实源单一化为 recipe 的 `QB_MACHINE`，笔误由 QEMU 的 unsupported-machine 直接暴露。它表达“这台 machine 应该如何被 QEMU 启动”，不同于记录 QEMU binary 来源和 sha256 的 `QEMU manifest`。
...
**remedy line**:
`ob` 在 exit 3（前置缺失）报错里给调用方（主要是智能 agent）的**下一步描述**。输出固定两段式：先**诊断行**说明哪条前置没满足（如 `Machine 'X' has not been initialized.`），再**恰好一行 remedy line**——描述满足该前置所需的下一步。常见形态是 `Run './ob init X' first.`（命令前缀随调用形态回显（`$0`），不 hardcode `ob`），但**不锁死为 ob 命令**：消费者是智能 agent，可自行决定用 ob 解决、通知用户或自行探索。要求非空且向前看（是「下一步」，而非「上一步可能失败了」这类纯回溯诊断）。**恰好一条命令、不串接第二条**：多步前置由单命令循环逐轮接力（补一步 → 重试 → 下一轮 remedy 指向下一步），而非一行列链（列链需调用方判断哪条才是当前步，违背无需推断原则）。
...
assignment-state 判定（"用户是否接管某 ob-managed variable"）的底层事实由 `read_local_conf_var` 的 exit code 承载（generic local.conf reader，不专属 ob-managed variable 集合）：`generate_build_config`（注入决策）与 `resolve_effective_dl_dir` / `resolve_effective_sstate_dir`（缓存目录 effective 路径解析 + mkdir）共用这一 exit code 判定，**不**用值判定（`-z` / `-n`）。本次只做 existing seam alignment；`ob-managed variable` 是否落成完整领域 module 经 [ADR-0014](docs/adr/0014-defer-generate-build-config-deepening.md) 判断为**现阶段不落成**（YAGNI：`.inc` 装配规则已由 orchestration/protocol 覆盖、`generate_build_config` 单调用方、无变更压力；前提改变见该 ADR 重新评估触发条件）。**effective-path 失败语义只适用路径类变量 DL_DIR / SSTATE_DIR**：有赋值行但值为空、或路径不可用（unwritable）→ `resolve_effective_*` 静默 `return 1`，调用点 `exit 3` + remedy（因 bitbake 会用空/无效路径破坏 fetch / sstate，且 ob 会把 bare mirror 填到 bitbake 不会用的位置）。`PREMIRRORS = ""` 仍是 ADR-0004 / ADR-0005 明确支持的合法禁用语义，不进路径 resolver、不触发 exit 3。
...
**ob deploy-to-qemu**:
ob 顶层的 image 级重建 + QEMU 重启命令，与 `start-qemu` / `stop-qemu` 同族（**非 `ob dev`**，归属见 [ADR-0011](docs/adr/0011-ob-deploy-to-qemu-toplevel-ownership.md)）。`ob deploy-to-qemu <machine>` 编排 build-first 链：`bitbake obmc-phosphor-image`（整个 image，1-4h）→ 若 QEMU 在跑则读旧 `QEMU PID file` 端口 + `qemu_instance_stop` + 注入 `QEMU_*_PORT` 全局做端口复用 → `qemu_prepare_launch` + `qemu_execute_launch`（新 image 跑在 QEMU）；没跑则 build + start。做"干净验证"——明确拒绝 recipe hot push（`devtool deploy-target` 那套，BMC 半新半旧、验证不权威）。exit-code 契约 0/1/2/3（0 = image 重建 + QEMU 启动；BMC-ready 超时只 warn 不算失败——deploy 核心交付是新 image 在 QEMU 跑起来，BMC ready 是 boot 过程）；用户交互式（stdout 透传 bitbake 日志 / QEMU 启动 / 连接 summary），与 `ob dev build`（agent-facing 空 stdout）**范式不同**。confirm 仅 QEMU 在跑时弹 banner（kill 运行中 QEMU 的路径风险，遵循 `confirmation banner` 术语），纯 build+start 无 banner（显式快路径）。deploy 抽象语义 = "让 target 跑上新代码做验证"，非字面"传文件"——故 v1 target 固定 QEMU（`-to-qemu` 后缀编码），未来真机部署另立 target 配置模型。与 `ob dev build` 正交：build 是 fast inner-loop 单 recipe 编译（秒-分钟，recipe 级，不碰运行态），deploy-to-qemu 是 image 级重建 + QEMU 重启（碰运行态）；workflow_02 引导两步模式（build 快速预失败 → deploy-to-qemu 完整验证）。自带编排调底层 module（`build_env_enter` / `bitbake` / `qemu_instance_*` / `qemu_prepare_launch` / `qemu_execute_launch`），**不调 cmd_build/cmd_start_qemu/cmd_stop_qemu**（零重构现有命令）。
_Avoid_: ob deploy（无后缀，暗示多 target 抽象，v1 不需要）, ob dev deploy（已退役 stub，误判归属）, recipe hot push / devtool deploy-target（状态不干净、验证不权威）, 把 BMC-ready 超时当 deploy 失败（只是 boot 慢）

**smoke suite**:
每 machine 必备的内建 suite——8 条服务可达性 AR（`SMOKE-01..08`：Redfish 根可达 / Redfish Managers 资源 / 固件版本非空上报 / IPMI mc info / SSH TCP 就绪门 / 串口 console 完整登录 / Web UI 根可达 / Web 登录 + 登录后会话资源），以纯数据分片 `ar_probes.d/smoke.yaml` 落在每个 baseline 目录、include 进薄顶层，经 `ob test-qemu <machine> --suite smoke` 运行（旧独立 smoke 顶层命令已收编退役，ADR-0028；两 machine AR 命名一致，便于 smoke_diff 跨 machine 配对）。语义内核沿袭旧命令：**probe-only**（不拥有 QEMU bring-up/teardown，死/stale `QEMU instance` → `qemu_instance_clean_stale` + exit 3，绝不探死端口报假「BMC 坏」）；verdict 纯报真相（runner 零 smoke 特判）；SSH TCP 就绪门自带 30×5s 有界轮询（超时判 fail 而非 error——可达性是 BMC truth）。temporal 回归闸门归 caller：`tools/smoke_regression.sh <machine> -- <change-cmd...>` 改前/改后各采一份 `--report` JSON 快照（**运行时临时产物 `mktemp`，非版本管理**）→ `tools/smoke_diff.py` 按 `records[].ar` 配对判定回归（pass→fail 同名退化 与 baseline 无此 AR + current fail 两类拦截；fail→pass 改善 info；skip/error 不参与——error 属 infra 非真相）→ exit 0 放行 / exit 1 拦截；ob test-qemu rc 3（前置缺失/infra）直接透传，不把 ERROR report 交给 diff 误放行。决策见 [ADR-0028](docs/adr/0028-smoke-merged-into-test-qemu-suite.md)，形态史见 [ADR-0020](docs/adr/0020-ob-smoke-probe-only-smoke-prober.md)。

**probe-type**:
baseline AR 的探测接口维度（`ar` 的 `probe` 字段，缺省 `redfish`）：`redfish`（HTTP/HTTPS 请求 + status/body/json_path 断言）、`ipmi`（ipmitool lanplus，凭据 env 注入、`-E` 形态密码不落 argv）、`ssh_tcp`（TCP 就绪门，无凭据）、`console`（pexpect 串口三阶段登录，socket/user/pass 走 env）、`web`（HTTP/HTTPS 静态资源 + status/content-type/body 断言；可选 `request.login` 数据驱动登录块——POST 建会话 → 带 cookie GET 目标路径 → best-effort logout，凭据走 `OB_TQ_WEB_*` env 链，SMOKE-08）、`none`（**planner-only sentinel**：仅 skip/cascade_skip AR 合法的「无可执行探测定义」标记，永不进入 runner probe 分派）。plan.py 按 probe-type 白名单校验 assert 原语兼容矩阵与 request schema（矩阵外组合 = 数据错 exit 3，不进 α truth）；runner 按 `probe` 字段分派 `probe_<type>.py`（probe seam 契约：stdout 一行 JSON + rc 0/1/3）；`cmd_test_qemu` 注入端口（`OB_TQ_SSH_PORT`/`OB_TQ_IPMI_PORT`/`OB_TQ_CONSOLE_SOCK`，来自实例 PID 文件）与 per-interface 凭据（`OB_TQ_IPMI_*`/`OB_TQ_CONSOLE_*`/`OB_TQ_WEB_*`，fallback 链 env > `auth.<interface>` > `auth` 顶层）。
_Avoid_: 独立 smoke 顶层命令（已退役，收编为 `ob test-qemu --suite smoke`，ADR-0028；勿再引用）, ob verify（历史名，"verify" 过载）

**ob test-qemu**:
ob 对一个**已在跑**的 `QEMU instance` 逐条跑该 machine 的 `baseline`（开发基线）里 QEMU 可仿真子集 AR 的独立顶层命令，同 probe-only 形态。**probe-only**：不 boot / 不 teardown（无 EXIT trap）。前置集按模式分叉：probe 模式前置是目标 machine 的 `QEMU instance` 正在跑——从 `QEMU PID file` 读真实 Redfish 转发端口（不接 port override，反映实际跑的实例）；`--dry-run` 是 baseline 资产检查（列 AR + applicability，不 probe），前置集仅 {machine、PyYAML、baseline 谱系+目录}，无需在跑实例、无需凭据（runner `DRY_RUN` 分支同构豁免）；probe 模式下无在跑实例（存活状态非 `running`）→ `qemu_instance_clean_stale` + exit 3 + `remedy line`（`Run './ob start-qemu <machine>' first`，绝不探死端口报假「BMC 坏」）。**baseline 目录谱系路由**（per-machine 全栈独立，[ADR-0025](docs/adr/0025-test-qemu-baseline-fullstack-per-machine.md) + [ADR-0026](docs/adr/0026-test-qemu-baseline-lineage-routing.md)）：谱系 = source label 单维度（唯一权威事实源——binary 目录/provisioning 均由 label 派生共线，binary 不构成独立信号）；label 缺失（`source manifest` 缺失/source_label 字段空）**不 fallback community**，判 unknown → exit 3 + 按成因 remedy（与写坏同档 fail-closed 强防御；binary provisioning 等其他 label 消费方的 community 缺失默认不受影响）；community 谱系 → 只查 `tests/baseline/<machine>/`（社区基线，随上游），custom 谱系 → 只查 `contexts/baseline/<machine>/`（custom 基线，不随上游），**各找各的、不跨谱系回退**，本谱系目录缺失 exit 3 + 按谱系指名 remedy；谱系判定与路由由 leaf-pure helper `test_qemu_lineage` / `test_qemu_resolve_baseline_dir` 实现（恒返回码 + outvar，对齐 `machine selection guard` / [ADR-0024](docs/adr/0024-qemu-instance-liveness-outvar.md)，可被 protocol 层直测、不必构造 fake alive QEMU）。路由命中**纯数据目录**后调主仓单副本 `tests/baseline/runner/run.sh`（薄 shim，编码钉死后 `exec python3 runner.py`；runner 引擎共享单副本，machine 差异只在数据 YAML，见 [ADR-0027](docs/adr/0027-baseline-runner-single-copy-in-main-repo.md)），数据路径由 cmd 以 `OB_TQ_AR_PROBES`/`OB_TQ_APPL` env 注入，auth 解析 env 优先（`OB_TQ_USER`/`OB_TQ_PASSWORD`，凭据全程不落 argv/`ps`，environ owner-only）、`ar_probes.yaml` 顶层 `auth:` 补缺（不硬编码凭据），host/port 由 cmd 注入；两份 YAML 顶层须声明 `schema_version`（plan.py 校验，不匹配 exit 3——runner 与数据分属两仓的版本门禁）；custom 谱系永不消费社区基线——custom build 的 fail 无法归因到上游（是上游也这样，还是 fork/build 特有？），路由层直接杜绝该错配，归因恒闭合。逐条 AR 经 probe + assert 原语判定（**runtime 确定性**，LLM 不参与判定），产 `pass` / `fail` / `skip` / `xfail` / `xpass` 五态 verdict（另：probe 取不到干净 BMC 应答的传输/超时/schema 错误记 `error`，属 infra、**不进 `fail` 的 α 统计**，见 exit 3）：`skip`（不适用 / 硬件依赖，**不调 probe**）、`xfail`（预期失败，**调 probe**：预期 fail→xfail、意外 pass→xpass，xpass 是改善信号）、`skip`/`xfail`/`xpass` **均不影响 exit**——只有 applicable 且实测不符的 `fail` 才 exit 1。`exit-code 契约` 0/1/2/3：0 全 applicable AR pass；1 ≥1 applicable fail = **α truth**（BMC 不满足 baseline 的真实一致性缺陷，**非 test-qemu broken**）；2 cancel；3 前置缺失（无在跑实例 / machine 未解析 / baseline 目录缺失）+ 一行 remedy，**或运行中 probe 命中瞬时 infra 错误（传输/超时/schema，VERDICT: ERROR）——此时实例未必坏，重跑 / 查连通性而非当 baseline 缺陷**。内建 `smoke suite`（ADR-0028 收编，8 AR 可达性门）守 per-push 绿灯；其余 suite 逐条深测、nightly / PR-to-main 频率。probe 按 `probe-type` 分派（redfish/ipmi/ssh_tcp/console/web），cmd 注入 per-interface auth（`auth.<interface>` 缺则 fallback 顶层 `auth`）与端口 env。LLM 角色限离线 authoring（读 Excel baseline → 翻译 AR 数据）与失败解说（fail 行自然语言解释），**均超出 spike 范围**（runtime 不参与判定）。
_Avoid_: conformance（弃用，见 `baseline` 词条 _Avoid_）, ob test-qemu verify / ob verify baseline（"verify" 过载，见 `smoke suite` 词条 _Avoid_）, 把 exit 1（α truth）当 test-qemu broken（fail 行报的是 BMC 实际行为，debug BMC 接口而非测试）

**baseline (开发基线 / 功能基线)**:
一份 BMC 固件开发基线文档（原始形态常为团队维护的 Excel），由若干带编号的需求条目（AR，如 `BMC-3-1-2` 密码规则）组成，是 `ob test-qemu` 在 QEMU 上做一致性验证的需求来源。每个 machine 的 baseline 数据自成体系、按项目独立迭代（不共享 AR 定义，见 [ADR-0025](docs/adr/0025-test-qemu-baseline-fullstack-per-machine.md)）；runner 引擎则为主仓单副本 `tests/baseline/runner/`（`tests/baseline/runner`，machine 差异只在数据 YAML，[ADR-0027](docs/adr/0027-baseline-runner-single-copy-in-main-repo.md) 修订 0025 的引擎不共享结论）。数据落点按分发边界二分：社区基线 `tests/baseline/<machine>/`（随 ob-harness 上游，**子仓纯数据**），custom 基线 `contexts/baseline/<machine>/`（不随上游，接 [ADR-0017](docs/adr/0017-knowhow-distribution-boundary.md) 的 `contexts/` 边界；只承载数据 + 数据生产线，零运行时代码）；`ob test-qemu` 按被测对象谱系硬路由二选一，不跨谱系回退（[ADR-0026](docs/adr/0026-test-qemu-baseline-lineage-routing.md)）。两仓（runner 主仓 / custom 数据子仓）由 YAML 顶层 `schema_version` 门禁耦合。QEMU 只覆盖其可仿真子集（纯软件 / 协议 / 网络档），硬件 / host 依赖档留给未实现的 `ob test-sut`。内建 `smoke suite`（旧 smoke 顶层命令收编，ADR-0028）守 per-push 绿灯，test-qemu 是逐条 pass/fail/skip/xfail/xpass 的深测（xfail 预期失败、xpass 意外通过，均不影响 exit）、全栈 per-machine。
_Avoid_: conformance（固件领域不用此词；既不专业也不通用，grill 中明确弃用）, AR 清单 / AR 集（AR 是 baseline 内的单条，baseline 是集合——用「baseline 条目」指单条，用「baseline」指整份）
...
`ob dev` 命令入口的解析+引导层 module（`lib/devtool_intake.sh`），把 shell 敲入的 `ob dev ...` argv 补全成 `cmd_dev` 可 dispatch 的 `(machine, subcmd, pattern, recipe)` 四元组。两条路径统一在这职责下：`dev_intake_argv`（确定性 argv 解析——`--machine`/二级子命令/`pattern`·`recipe` 上下文派生/token 合法性守卫，return 0/1）与 `dev_intake_tty`（仅当子命令缺失且交互终端时进入的 7 项菜单引导 + 各子命令位置参数补齐 + 衔接 `modified recipe selection`，return 0/1/2/3）。它是 leaf-pure L3（绝不 exit，return `exit-code 契约`值，exit 收口独占留 `cmd_dev`，对齐 ADR-0010/0012），函数前缀 `dev_intake_*`（与 `dev_dispatch_subcmd`/`dev_subcmd_*` 同族）。与 `subcommand handler` 正交且相邻：intake 把命令补全成可 dispatch 的字段，handler 消费字段编排 execute module——三层 intake → handler → run（`devtool_*_run`）都是 L3 leaf-pure 领域化。parsed fields 经 nameref outvar 回传（不污染全局，区别于历史 `pick_machine` 设全局 `$MACHINE`）。
_Avoid_: 命令信封/envelope（dead metaphor，曾用）, parser（窄，不含 TTY 引导段）, command entry/入口（口语化）, command resolver（太泛）, 把 intake 当 subcommand handler（intake 解析命令入口，handler 编排子命令执行）

**ob init command intake**:
`ob init` 命令入口的解析+引导+确认层 module（`lib/init_intake.sh`），把 shell 敲入的 `ob init ...` argv 解析+确认为 `cmd_init` 可消费的 ready-to-init machine。封装三路决策：fastpath（命令行给定合法 machine → 不 confirm，对齐 `confirmation banner` 显式快路径）/ pick+confirm（交互选出 → 弹 confirm）/ 前置缺失（`list_available_machines` 空 / 非 TTY / arg 非法且非 TTY）。`init_intake` return `exit-code 契约`值 0/1/2/3（0=解析+确认成功 / 1=读失败 / 2=用户取消[pick 或 confirm 任一] / 3=前置缺失），exit 收口独占留 `cmd_init`（字面 case 映射，对齐 `cmd_dev` L435-438/L445-446），leaf-pure L3。三条 load-bearing 设计决策：(1) **`confirm_action` 划进 intake**——"何时 confirm"完全由解析路径决定（fastpath 不弹 / pick 路径弹），与其唯一信息源同处一层、接口最深，区别于 `cmd_build` 的 confirm 留 L1；(2) **沿用 `$MACHINE` 既有全局、不引入 nameref outvar**——`pick_machine` 设全局无法消除（改它超本次范围，`machine selection` 是稳定接口）、`$MACHINE` 本就是 ob 既有全局，不造 outvar 假对称，区别于 `ob dev command intake` 的 nameref 不污染全局（recipe 是局部概念、`$MACHINE` 是全局，不可比）；(3) **empty/nontty 检测原样保留**（empty 前置 + nontty 后置 else）、不复用 `machine selection guard`（Phase 2 暂缓，见 [ADR-0016](docs/adr/0016-defer-init-intake-guard-reuse.md)）。与 `ob dev command intake` 同构但不对称：dev 的 intake 不含命令级 confirm（dev 无重操作 gate）、用 nameref outvar 回填局部；init 的 intake 含 confirm（init 是重操作）、沿用 `$MACHINE` 全局——不对称源于两命令的语义差异（重操作 vs 轻量内循环），非设计不一致。intake 边界落在 `confirm_action` 通过、`info "Init confirmed"` 打印之后；`BUILD_DIR/SRC_DIR` 重派生、`machine_state_clear_init_progress`、`devtool_recipes_clear_cache`、rerun 探测留 `cmd_init`（副作用 + exit 1 路径属 L1 编排，不下沉进 leaf-pure intake）。
_Avoid_: ob init parser（窄，不含 confirm 引导段）, 把 confirm 当 L1 gate 留 cmd_init（confirm 划进 intake）, 引入 outvar 回填 machine（沿用 `$MACHINE` 全局）, 把 intake 当 init_pipeline 的 step 函数（它是入口解析层，非执行编排）, 强求复用 `machine selection guard`（Phase 2 暂缓，见 ADR-0016）, 把 init intake 与 dev intake 的不对称当设计缺陷（源于命令语义差异）
