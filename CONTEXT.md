...
**patch landing (落回)**:
`ob dev finish` 把 `srctree` 改动写回 recipe 的落回结果，由 finish porcelain 的五个字段表达：`landing_mode`（"patch" 生成 `.patch` 加进 `SRC_URI` / "srcrev" 更新 `SRCREV` / `null` 当 noop）；`landing_layer`（落回的目标 layer，ob 取 recipe 原属 layer）；`patches`（patch mode 新增 `.patch` 路径列表，srcrev/noop 为 `[]`）；`recipe_files`（被 finish 修改的 layer recipe 文件 `.bb`/`.bbappend` 路径列表，noop 为 `[]`）；`srcrev`（srcrev mode 新 `SRCREV`，patch/noop 为 `null`）。落回结果不由 parse `devtool finish` 自由文本 stdout 获得，而由 ob 在 devtool finish 前后快照 `landing_layer` 文件树 diff 探测（在 ob dev 单 writer 假设下可靠）。与 `srctreebase`/`disposition`（源码副本物理去向）正交。
