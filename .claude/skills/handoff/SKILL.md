...
提炼硬规则：
- 用户请求与显式约束沿用『硬约束』中的 materiality 过滤 + verbatim 复制规则（过滤后为空写 `无`）；此外不要把多个原话拼成一句概括，也不要改写保留下来的原话；已过期、对继续工作不再重要的引用内容要省略
...
- 重新审视已引用的用户请求或约束：对继续工作是否仍然真正重要；不再重要的应弱化或省略（区别于『硬约束』里的初次 materiality 过滤，这是二次复核）
- 下一个 chat 的第一步应该做什么
