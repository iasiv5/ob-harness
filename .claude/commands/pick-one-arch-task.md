...
# Pick One Architecture Task (Claude Code Entry)

这是 Claude Code 的 `/pick-one-arch-task` slash command 入口。本文件用于约束 Claude Code 只产出**唯一一项**最值得今天就开工的架构优化任务，避免泛泛的代码审查或一次性堆砌多个候选。重点是**选题质量**。
