...
| 忘记更新 SKILLS_INDEX.md | 新 skill 没人能找到 | 写完 skill 后立即更新 `rules/05_SKILLS_INDEX.md` |

## 与现有 skill 的关系

写新 skill 之前，先读 `rules/05_SKILLS_INDEX.md` 确认没有重复。如果已有类似 skill，优先修改而非新建。

格式参考可以看 `rules/skills/workflow_parallel_subagents.md`（工作流类 skill 的范本）和 `rules/skills/bestpractice_temporal_info_verification.md`（最佳实践类 skill 的范本）。注意这些只是格式参考，核心原则（结果确定性、enabling 而非 SOP）比格式更重要。
