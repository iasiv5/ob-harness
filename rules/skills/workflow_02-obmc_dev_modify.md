...
- **类型**: Workflow
- **适用场景**: 改 OpenBMC 某 recipe 的源码（devtool modify 阶段）：把 recipe 源码检出到本地、改、增量 build；image 级验证用 `ob deploy-to-qemu`（规划中，独立工作项）
- **创建日期**: 2026-07-13
...
**ob dev 不做**（agent 自由区）：手改 recipe 元数据（.bb）、解析 bitbake 日志、recipe 依赖图分析、批量 modify、部署（预留，待 ob 提供）。
