...
解决 `set -euo pipefail` 下管道命令意外中止：pipefail 把下游"信号性非零"（`grep` 无匹配返回 1、`head` 提前关闭返回非零）误判为硬错误触发 `set -e`。两条最该先知道的：(1) 裸 `cmd | grep -q`（没被 `if` / `|| true` 包裹）一律是 bug 嫌疑——pipefail 无法区分"信号性非零"和"失败性非零"，判定权必须交回写脚本的人；(2) 别用 `|| true` 一刀切——会连真实失败（如 `curl` 挂了）一起吞掉，只对"下游非零是预期信号"的管道用。三类处置（纯布尔判断 / 保留输出允许无匹配 / 下游提前关闭）见 §处置模式；无命令 `exec` 持久重定向、errexit 禁用上下文吞失败、`< 文件` 打开失败绕过 `2>/dev/null` 是同族相邻陷阱（见尾部三段）。
