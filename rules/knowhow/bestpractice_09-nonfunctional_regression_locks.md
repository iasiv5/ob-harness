...
为"不改输出但声称更好"的改动（性能 / 去重 / 缓存）加**可回归验证的硬约束**：不只验证结果对，还验证以期望方式达成（调用了几次 / 是否走快路径），防优化悄悄回退。三条最该先知道的：(1) 计数写**文件**不写变量——`$()` / `< <()` 子 shell 里变量累加会丢、永远 0；(2) 期望值钉死 `==N` 不是 `>=N`——松断言发现不了回退，等于没断言；(3) 只对"输出不变"的非功能性改动加，**别**对功能性改动滥用（合法重构换实现时测试会误红）。两种模式（调用次数断言 / 零调用断言）见 §处置模式。
