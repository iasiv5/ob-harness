...
知识工程方法论：**笨数据 + 笨方法 + 笨模型 = 精知识**——接受起点数据不完美，靠可迭代飞轮逐步提纯知识纯度。四步飞轮循环：**触发**（识别微小、可验证的子问题）→ **调用基础模块**（线性扫描 / 语义检索 / 结构化输出等"笨方法"）→ **产生微小进步**（提取一个实体关系 / 澄清一个概念）→ **精炼**（固化进知识库，成为下一轮基础模块）。三条最该先知道的：(1) 选笨方法不选过度设计——先用线性扫描跑起来，再谈索引优化；(2) 本地部署开源模型（如 Qwen3-32B INT4，边际成本接近零），不选按 token 计费的闭源 API；(3) 避开四大陷阱：过度设计索引 / 追求完美数据 / 复杂 pipeline（每增一环增一失败点）/ 过早优化——先有 1.0 再谈优化。
...
| 日期 | 变更 |
|------|------|
| 2026-02-21 | 从 OBSERVATIONS.md 晋升，整理为独立 know-how |
| 2026-07-25 | 引入 ob-harness `rules/knowhow/`，编号 workflow_03（随 skill→know-how 改名，见 ADR-0013） |
