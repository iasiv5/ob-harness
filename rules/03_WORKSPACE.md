...
- `ob` 模块化主体：`lib/`（`util.sh` 底层工具 / `repo.sh` 仓库解析 / `bitbake_env.sh` BitBake environment one-shot 查询 / `qemu.sh` QEMU runtime / `machine_state.sh` 生命周期状态 / `init_pipeline.sh` init 流水线 / `commands.sh` cmd_* 编排；文件边界即 `function semantic layer`，见 `CONTEXT.md`）
