...
- OpenBMC 环境初始化工具：根目录 `ob`（`./ob init [<machine>]` 一键初始化；`./ob dev [--machine <m>] <list|modify|build|refresh|reset|status|finish>` recipe 源码开发；`./ob deploy-to-qemu [<machine>]` image 级重建 + QEMU 重启做干净验证，与 ob dev build 正交）（环境生命周期动作先走 ob，见 `rules/skills/bestpractice_06-ob_first.md`）
