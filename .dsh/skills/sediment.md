---
name: sediment
description: 手动经验沉淀入口：判所有权 → 判层 → 写 → 收口（执行合同见 rules/knowhow/workflow_04-manual_sedimentation.md）。
---

# Sediment (DeepSeek Harness Entry)

这是 DeepSeek Harness 的 `/sediment` 斜杠命令入口。本文件只负责引用执行合同。
